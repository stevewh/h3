...
/*	$bkmkID = $res->fetch_assoc();
	if ($bkmkID['rec_OwnerUGrpID']  &&  $bkmkID['rec_NonOwnerVisibility'] == 'hidden') {
		$res = $mysqli->query('select * from '.USERS_DATABASE.'.sysUsrGrpLinks where ugl_GroupID='.intval($bkmkID['rec_OwnerUGrpID']).' and ugl_UserID='.get_user_id());
