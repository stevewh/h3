...
*/
  ...
  if (!defined('MEMCACHED_SERVER')) define('MEMCACHED_SERVER', 'mcachesrv');
    ...
    global $mysqli;
		...
		if ($mysqli->error) {
			error_log("queryError in getSearchResults -".$mysqli->error);
		}
		$fres = $mysqli->query('select found_rows()');
		$resultCount = mysqli_fetch_row($fres); $resultCount = $resultCount[0];
		...
		if ($onlyIDs) {
			...
			while ($row = $res->fetch_assoc()) {
				...
				if ($mysqli->error) {
					error_log("2.queryError in getSearchResults -".$mysqli->error);
				}
...
*/
    ...
    global $mysqli;
		...
		foreach ( $recIDs as $recID ){
			...
			$rectype = mysqli__select_array($mysqli, "Records","rec_RecTypeID","rec_ID = $recID");
			...
			if ($rectype == $colRT) { // collection rec so get query string and expand it and list all ptr recIDs
				$qryStr = mysqli__select_array($mysqli, "recDetails","dtl_Value","dtl_DetailTypeID = $qStrDT and dtl_RecID = $recID");
				...
				$collRecIDs = mysqli__select_array($mysqli, "recDetails","dtl_Value","dtl_DetailTypeID = $resrcDT and dtl_RecID = $recID");
...
//		if (! $memcached) {
    ...
    global $mysqli;
		...
		$record["tags"] = mysqli__select_array($mysqli, 
		...
		$record["wgTags"] = mysqli__select_array($mysqli, 
