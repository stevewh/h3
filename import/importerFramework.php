...
function find_similar_entries(&$entry) {
	...
	if (HASH_PREFIX_LENGTH) {
		...
		$similar_query = "select rec_ID as matching_bib_id, NEW_LEVENSHTEIN($hashColumn, upper('" . addslashes($hash) . "')) as lev from Records where ! rec_FlagTemporary and rec_RecTypeID = " . $entry->getReferenceType() . " and $hashColumn like '" . addslashes($hprefix) . "%' having lev is not null order by lev";
	}
	else {
		$similar_query = "select rec_ID as matching_bib_id, NEW_LEVENSHTEIN($hashColumn, upper('" . addslashes($hash) . "')) as lev from Records where ! rec_FlagTemporary and rec_RecTypeID = " . $entry->getReferenceType() . " having lev is not null order by lev";
	}
...
'
   select sum(rst_RecordMatchOrder) as bdr_match_count
     from recDetails BD1
left join recDetails BD2 on BD1.dtl_DetailTypeID=BD2.dtl_DetailTypeID 
          and (BD1.dtl_Value=BD2.dtl_Value 
               or (length(BD1.dtl_ValShortened) > 20
                   and cast(NEW_LIPOSUCTION(BD1.dtl_Value) as char) = cast(NEW_LIPOSUCTION(BD2.dtl_Value) as char)))
left join Records on BD1.dtl_RecID=rec_ID
left join defRecStructure on rst_DetailTypeID=BD1.dtl_DetailTypeID
                             and rst_RecTypeID=rec_RecTypeID
...
<?php
	$res = $mysqli->query('select rec_ID,rec_Title,
	                           NEW_LEVENSHTEIN(rec_Hash,upper("'.addslashes($ambig_entry->getHHash()).'")) as diff1,
	                           NEW_LEVENSHTEIN(upper(rec_Title),upper("'.addslashes($ambig_entry->getTitle()).'")) as diff2
