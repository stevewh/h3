...
function SelectTerms(_isFilterMode, _isWindowMode) {
  ...
  termLabelIndex = top.HEURIST.terms.fieldNamesToIndex['trm_Label'],
  termConceptIndex = top.HEURIST.terms.fieldNamesToIndex['trm_ConceptID'],
  params, // paramerters passed
  _listName, // name passed in to display
  existingTree = [],
  ...
  function _init(_callback) {
    ...
    if((location.search.length > 1)) {
      params = top.HEURIST.parseParams(location.search);

      _listName = "<h2 class='dtyName'>" + (params.name ? params.name : "List Name Not Supplied") + "</h2>";
      Dom.get("dtyName").innerHTML = _listName;

      if(params.selonly == "1"){
        Dom.get("headerDiv").style.display = "none";
        Dom.get("selectedTermsTree").className = "selectedTerms";
      }
      _datatype = (params.datatype ? params.datatype :'enum');
      _allTerms =  (params.all? params.all : null);
      _disTerms = (params.dis? params.dis : null);
  ...
  */
  ...
  function _createPreview() {
    ...
    //output strings
    var _termsJSON = YAHOO.lang.JSON.stringify(termIDTree);
    var _disTermsString = YAHOO.lang.JSON.stringify(disabledTermsList);
    if(top.HEURIST.util.countObjElements(termIDTree) > 0) {
      var termName, termCode, parentTermCode, treeString="";
      for (termID in termIDTree) {
        termName = termsByDomainLookup[termID][termLabelIndex];
        termCode = termsByDomainLookup[termID][termConceptIndex];
        if (treeString) treeString += ",";
        treeString += termName + ":" + termCode;
        if(top.HEURIST.util.countObjElements(termIDTree[termID]) > 0) {
          parentTermCode = termCode;
          for (subtermID in termIDTree[termID]) {
            termName = termsByDomainLookup[subtermID][termLabelIndex];
            termCode = termsByDomainLookup[subtermID][termConceptIndex];
            treeString += "," + termName + ":" + termCode + ":" + parentTermCode;
           }
        }
       }
    }
    Dom.get("termTreeJSON").innerHTML =  treeString;
    Dom.get("selectedTermsString").innerHTML = _disTermsString;
  }
  ...
  */
  ...
  function _addNewTerm(){
    ...
    var db = (params.db? params.db : (top.HEURIST.database.name?top.HEURIST.database.name:''));
    ...
    Hul.popupURL(top, top.HEURIST.basePath + "admin/structure/editTerms.php?popup=1&db="+db+"&domain="+_datatype,
      {
        ...
        callback: function(needTreeReload) {
          if( needTreeReload) { //refresh in any case
