...
foreach ($textDetails as $textDetail) {
	if (! check($textDetail['dtl_Value'])){
		if ($prevInvalidRecId < $textDetail['dtl_RecID']) {
			...
			mysqli__update($mysqli, "Records", "rec_ID=".$textDetail['dtl_RecID'],array("rec_Modified" => $now));
		}
		...
		mysqli__update($mysqli, "recDetails", "dtl_ID=".$textDetail['dtl_ID'], array("dtl_Value" =>$newText));
		if ($mysqli->error) {
			print "<tr><td><pre>" . "Error ". $mysqli->error."while updating to : ".htmlspecialchars($newText) . "</pre></td></tr>\n";
		}else {
